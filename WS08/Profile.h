...
namespace sdds
{
	...
	struct Profile
	{
		...
		{
			bool addressNumberNegative = m_address.number < 0;
			bool validPostal = (isalpha(m_address.postal_code[0]) && isdigit(m_address.postal_code[1]) && 
								isalpha(m_address.postal_code[2]) && m_address.postal_code[3] == ' ' && 
								isdigit(m_address.postal_code[4]) && isalpha(m_address.postal_code[5]) && 
								isdigit(m_address.postal_code[6]));
			

			if (addressNumberNegative)
			{
